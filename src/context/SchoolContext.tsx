...
export const SchoolProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const saveData = async () => {
      try {
        ...
        await AsyncStorage.setItem(STORAGE_KEYS.SCHOOL_DATA, JSON.stringify(dataToSave));
      } catch (e) {
  ...
  const addSchool = useCallback(async (school: Omit<School, "id" | "countClasses">) => {
    try {
  ...
  const updateSchool = useCallback(async (id: string, school: Partial<School>) => {
    try {
      ...
      setSchools((prev) => prev.map((s) => (s.id === id ? { ...s, ...school } : s)));
    } catch (err) {
  ...
  const fetchClasses = useCallback(async (schoolId: string, refresh = false) => {
    const page = refresh ? 1 : classPage;
    const hasLocalClasses = classes.some((c) => String(c.schoolId) === String(schoolId));
    ...
    try {
      const data = await classService.getClasses(schoolId, page, PAGINATION.DEFAULT_PAGE_SIZE);
      ...
      setClasses((prev) => {
        ...
        return Array.from(new Map(combined.map((c: any) => [c.id, c])).values());
      });
  ...
  const updateClass = useCallback(async (id: string, schoolClass: Partial<SchoolClass>) => {
    try {
      await classService.updateClass(id, schoolClass);
      setClasses((prev) => prev.map((c) => (c.id === id ? { ...c, ...schoolClass } : c)));
    } catch (err) {
  ...
  const contextValue = useMemo(() => ({
    schools, classes, isLoading, isLoadingMore, error,
    schoolPage, hasMoreSchools, totalSchools, classPage, hasMoreClasses,
    ...
    schools, classes, isLoading, isLoadingMore, error,
    schoolPage, hasMoreSchools, totalSchools, classPage, hasMoreClasses,
    addSchool, updateSchool, deleteSchool, addClass, updateClass, deleteClass,
    fetchSchools, fetchClasses,
  ]);
