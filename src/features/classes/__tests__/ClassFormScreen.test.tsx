...
jest.mock("../../../hooks/useClasses", () => ({
  useClasses: jest.fn(() => ({
    allClasses: [{ id: "c1", name: "Old Class", shift: "Morning", academicYear: "2024" }],
...
describe("ClassFormScreen", () => {
  ...
  it("renders for new class and adds it", async () => {
    const { getByPlaceholderText, getByTestId, getByText } = render(<ClassFormScreen />);
    ...
    await waitFor(() => {
      expect(mockAddClass).toHaveBeenCalledWith(expect.objectContaining({
        schoolId: "1",
        name: "New Class",
        academicYear: "2025",
      }));
  ...
  it("renders for editing class and updates it", async () => {
    ...
    (useLocalSearchParams as jest.Mock).mockReturnValue({ id: "1", classId: "c1" });
